...
class FixtureApp:
    def __init__(self, root):
        ...
        # Cargar el ícono de la ventana si existe
        if os.path.exists(self.ruta_icono):
            try:
                self.root.iconbitmap(self.ruta_icono)
            except Exception:
                pass
        # ----------------------------------
        ...
        if not os.path.exists(self.carpeta_banderas):
            try:
                os.makedirs(self.carpeta_banderas)
            except Exception:
                pass
    ...
    def mostrar_info(self):
        messagebox.showinfo("Información", "App para el mundial hecho con ❤️ por TUTE + Gemini")
        ...
        for grupo, equipos in grupos_equipos.items():
            for equipo in equipos:
                ...
                if os.path.exists(ruta_img):
                    try:
                        self.dict_banderas[equipo] = tk.PhotoImage(file=ruta_img)
                    except Exception:
                        pass
    ...
    def setup_tab_grupos(self):
        ...
        self.frame_partidos.pack(fill="x", padx=20, pady=(5, 20))
    ...
    def setup_tab_eliminatorias(self, fase_actual, frame_tab, fase_siguiente):
        ...
        # Frame que contendrá todos los partidos
        scroll_frame = tk.Frame(canvas, bg="#264d3b")
        
        # --- SOLUCIÓN PARA EL SCROLL ---
        def _on_mousewheel(event):
            # Solo scroll si el mouse está sobre este canvas específico
            canvas.yview_scroll(int(-1*(event.delta/120)), "units")
        
        # Solo enlazamos el evento cuando el mouse entra en el canvas
        canvas.bind("<Enter>", lambda e: canvas.bind_all("<MouseWheel>", _on_mousewheel))
        # Desenlanzamos cuando el mouse sale para no interferir con otras partes de la app
    ...
    def refresh_eliminatoria_ui(self, fase_actual):
        ...
        for i, p in enumerate(self.llaves[fase_actual]):
            ...
            # Verifica si hay empate para mostrar las cajas de penales
            is_tie = (str(p.get("res_l", "")) == str(p.get("res_v", "")) and str(p.get("res_l", "")) != "")

            # --- PENALES LOCAL (Oculto si no hay empate) ---
            ...
            # --- PENALES VISITANTE (Oculto si no hay empate) ---
    ...
    def auto_avanzar_eliminatoria(self, fase_actual, idx, fase_siguiente, el, ev, e_pl, e_pv):
        ...
        # Leemos los penales si los casilleros existen y tienen algo
        ...
        # Si no cambió nada de nada, salimos
        ...
        # Si pasamos de no-empate a empate (o viceversa), refrescamos la UI para mostrar/ocultar cajas amarillas
        if was_tie != is_tie:
            self.refresh_eliminatoria_ui(fase_actual)
            return  # Cortamos acá para que el usuario pueda escribir los penales tranquilamente
            
        if is_tie:
            # Si hay empate pero faltan los penales o están empatados también, no avanzamos a nadie aún
            if pl == "" or pv == "" or pl == pv:
                return 
            ganador = p["l"] if pl > pv else p["v"]
        else:
            # Si no hay empate, limpiamos los penales por las dudas
            p["pen_l"], p["pen_v"] = "", ""
            self.guardar_progreso_disco()
